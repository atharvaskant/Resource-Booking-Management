...
include ('dbconfig.php');
    ...
    $query="select * from users1 where Email_ID='$email' AND Password='$password'";
    $result=mysqli_query($con,$query);

    // Extracting all details of user
    $nquery="select * from users1 where Email_ID='$email'";
    ...
    $sql="select * from users1 where Department='$dept' AND Designation='HOD'";
    $r=mysqli_query($con,$sql);
    $row1=mysqli_fetch_assoc($r);
    $_SESSION['concerned']=$row1['Email_ID'];


    $psql="select * from users1 where Designation='Principal'";
